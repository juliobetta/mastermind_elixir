defmodule MasterMind.Web.GameChannelTest do
  ...
  import MasterMind.Application, only: [generate_game_id: 0, generate_player_id: 0]
  ...
  test "play", %{socket: socket, game_id: game_id} do
  ...
  test "leaving the game channel kills the game", %{game_id: game_id, game: game, socket: socket} do
    ...
    assert {:error, "Game does not exist"} = GameServer.join(game_id, @player_id, socket.channel_pid)
  end
end
